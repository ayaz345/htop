...
selfheader = f'#include "{name}.h"'

out.write( "/* Do not edit this file. It was automatically generated. */\n" )
out.write( "\n" )

out.write(f"#ifndef HEADER_{os.path.basename(name)}" + "\n")
out.write(f"#define HEADER_{os.path.basename(name)}" + "\n")
is_blank = False
for line in file.readlines():
   ...
   if state == ANY:
      if line == '/*{':
         ...
         equals = line.find(" = ")
         if line[-3:] == "= {":
            out.write(f"extern {line[:-4]}" + ";\n")
            state = SKIP
         elif equals != -1:
            out.write(f"extern {line[:equals]}" + ";\n")
      ...
      if len(line) >= 1 and line[0] == "}":
         state = SKIPONE if static == 1 else ANY
         static = 0
   elif state == SKIPONE:
      is_blank = False
      state = ANY
...
out.seek(0)
try:
   with io.open(f"{name}.h", "r", encoding="utf-8") as orig:
      origcontents = orig.readlines()
except:
   origcontents = ""
if origcontents != out.readlines():
   with io.open(f"{name}.h", "w", encoding="utf-8") as new:
      print(f"Writing {name}.h")
